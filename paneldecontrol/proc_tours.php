...
$cnt_tours = new cls_tbl_tours($id);
$cnt_tours->settxt_nombre($_POST['get_txtnombre']);
$cnt_tours->setfk_destino($_POST['sle_destino']);
$cnt_tours->settxt_descripcion($_POST['get_content']);
$cnt_tours->settxt_status($_POST['chktours']);
...
$cnt_tours->settxt_creacion($thisdatedepa);

$imagen = $_POST['h_thumbtours'];
if ($_FILES['fileup_tours']['name']==NULL)
{$cnt_tours->settxt_imagen($imagen);}
else
{
$max_size = 1024*1000; #<>5Mb
$my_upload = new file_upload;
$my_upload->upload_dir = ADMIN_PHOTOBIG_TOURS;
$my_upload->extensions = array(".jpeg",".jpg");
$my_upload->max_length_filename = 50;
$my_upload->rename_file = true;
$my_upload->the_temp_file = $_FILES['fileup_tours']['tmp_name'];
$my_upload->the_file = $_FILES['fileup_tours']['name'];
$my_upload->http_error = $_FILES['fileup_tours']['error'];
...
$new_name = "";
		...
		$w = $infoimg[0]; #
			if($w>300){
				...
				$resize_image->resize(W_TOURS_MAX,H_TOURS_MAX);
				$resize_image->save($full_path); #save(path,100) => 100 <> a la calidad de la imagen
				$cnt_tours->settxt_imagen($my_upload->file_copy);
			}
		}else{
		$cnt_tours->settxt_imagen($imagen);
		}
...
switch($op)
{
	case 1:	//guardar
			$cnt_tours->guarda();
			$idsec=$cnt_tours->getpk_tours();
	break;
	case 2://Actualizar	
			$cnt_tours->setpk_tours($id);	
			$depa_borrar= new cls_tbl_tours($id);						
			$file_temp = $depa_borrar->gettxt_imagen();				
			if($file_temp!=$cnt_tours->gettxt_imagen())
			{	deleteFiles(ADMIN_PHOTOBIG_TOURS,$file_temp);	}									
			$cnt_tours->actualiza();
...
/*			echo $op;
	...
	case 3://Eliminar	
			$cnt_tours = new cls_tbl_tours($id);
			deleteFiles(ADMIN_PHOTOBIG_TOURS,$cnt_tours->gettxt_imagen());
			$cnt_tours->elimina();					
	...
	{	
		...
		{		if($_POST["chktours"])
				{	$sorteo = new cls_tbl_tours($valor);
					@$sorteo->estado("1");				
				}
		}		// foreach
	}
	...
	if(!empty($_POST["chktours"]))
	{	
		foreach($_POST["chktours"] as $id=>$valor)
		{		if($_POST["chktours"])
				{	$sorteo = new cls_tbl_tours($valor);
					@$sorteo->estado("0");				
				}
