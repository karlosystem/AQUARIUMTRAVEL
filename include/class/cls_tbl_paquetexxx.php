...
$SQL = "SELECT * FROM [|PREFIX|]paquete Inner Join [|PREFIX|]paquete_images ON [|PREFIX|]paquete.pk_paquete = [|PREFIX|]paquete_images.prod_id
WHERE [|PREFIX|]paquete_images.prod_id =  '".$this->getpk_paquete()."' ORDER BY [|PREFIX|]paquete_images.prod_order ASC ";
}else{
$SQL = $QueryParam;
}
...
{
	if(!tep_not_null($sql)){
	$sql = $GLOBALS['CONNECT_DB']->Query("SELECT pk_paquete, txt_datepaquete, fk_categoria, txt_youtube, int_status, txt_dateadd, txt_dateupdate, txt_date_from, txt_date_to, int_dias, int_noches, txt_precio, int_ishome, int_isnuevo, [|PREFIX|]paquete_details.txt_pdf, fk_destino, txt_precio_soles, txt_tipo_cambio, int_isdestacado, LENGTH(txt_meta_description) as caracteres FROM [|PREFIX|]paquete LEFT JOIN [|PREFIX|]paquete_details ON [|PREFIX|]paquete.pk_paquete = [|PREFIX|]paquete_details.fk_paquete ORDER BY [|PREFIX|]paquete.pk_paquete ASC");
	}else{
...
function get_paquete_detail($pak_id, $language_id = 0) {
    global $languages_id;
...
function ListPaquetesCategoria($from = 0, $to = 20, $page = 1){
 ...
 if($count >= 1) {
	...
	while($Fetch = $GLOBALS['CONNECT_DB']->Fetch($Query)) {
		 ...
		 $Disponible = $Fetch['int_disponible'];
		
if(_SEOMOD==1){
			$link_paquete = _URL."paquetes/tours-".clean_url($Title)."-pid-".$Fetch['pk_paquete']."."._FEXT;
		}else{
			$link_paquete = _URL.'paquete_detalle.php?pid='.$Fetch['pk_paquete'];
	}
         ...
         $strgal_paquete .= "                  <span class=\"PricepriceWithoutTax\" >Tarifas Desde:</span>";
         $strgal_paquete .= "              </div>";		
		 
		 $strgal_paquete .= "				<div class=\"PricesalesPrice\" style=\"display : block;\">";
		 $strgal_paquete .= "					<span class=\"PricesalesPrice\" >$ ".(int)$Fetch['txt_precio']."</span>";
         $strgal_paquete .= "               </div>";
         ...
         $strgal_paquete .= "              <label for=\"quantity\" class=\"quantity_box\">Todo Incluido </label>";
         $strgal_paquete .= "              <span class=\"quantity-controls\">";
		 $strgal_paquete .= "              </span>";
 		 $strgal_paquete .= "			   <div class=\"clear\"></div>";
         $strgal_paquete .= " 					<span class=\"addtocart-button\">";
         $strgal_paquete .= "						<input type=\"submit\" class=\"addtocart-button cart-click\" value=\"DETALLES\" title=\"Add to Cart\" />";
		 $strgal_paquete .= "					</span>";
         $strgal_paquete .= "              </div>";                                              
         $strgal_paquete .= "			</form>";
         $strgal_paquete .= "           </div>";                                    
		 ...
		 $strgal_paquete .= "			<div class=\"browseProductImageContainer\">";
 		 $strgal_paquete .= "				<a href=\"#\" class=\"img-scr\">";
		 ...
		 $strgal_paquete .= "	<h2><a href=\"$link_paquete\">".$Title."</a></h2>";
		 $strgal_paquete .= "	<p>".fewchars($Meta,300)."</p>";																					
		 ...
		 $strgal_paquete .= "			<a href=\"$link_paquete\" class=\"product-details\">Mas Informacion</a>";
		 $strgal_paquete .= "		</div>";   */                                         
		 $strgal_paquete .= "	</div>";
		 $strgal_paquete .= "</div>";
		 $strgal_paquete .= "<div class=\"clear\"></div>";
...
if($languages->IsExistLanguage($languages_id)) {
 ...
 $SQL = "SELECT [|PREFIX|]paquete.txt_precio, [|PREFIX|]paquete_details.fk_paquete, [|PREFIX|]paquete.int_ishome, [|PREFIX|]paquete_details.txt_detalle, [|PREFIX|]paquete_details.txt_title, [|PREFIX|]paquete_details.txt_incluye, [|PREFIX|]paquete.fk_categoria FROM [|PREFIX|]paquete ";
 ...
 while($Fetch = $GLOBALS['CONNECT_DB']->Fetch($Query)){
	...
	$title_paquete = tep_output_string($Fetch['txt_title']);
	if(_SEOMOD==1){
			$link_paquete = _URL."paquetes/tours-".clean_url($title_paquete)."-pid-".$Fetch['fk_paquete']."."._FEXT;
		}else{
			$link_paquete = _URL.'paquete_detalle.php?pid='.$Fetch['fk_paquete'];
	}
	...
	$str_destacado .= "		<a href=\"$link_paquete\" class=\"img2\">";
	$str_destacado .= tep_image(_URL.'resize.php?image='.$img_thumb.'&w=220&h=220&IsCrop=0',$Title,'','','class="browseProductImage featuredProductImage"');
	
	$str_destacado .= "	    </a>";
	$str_destacado .= "		<div class=\"Price\">";
	$str_destacado .= "			Desde:<span class=\"sales\">$ ".$Fetch['txt_precio']."</span>";
	$str_destacado .= "			<span class=\"WithoutTax\">$100.00</span>";
	$str_destacado .= "		</div>";
	$str_destacado .= "	</div>";
	$str_destacado .= "	<div class=\"fleft\">";

	$str_destacado .= "		<div class=\"description\">";
	$str_destacado .= "			<p>".$title_paquete."</p>";
	$str_destacado .= "		</div>";
	
/*	$str_destacado .= "		<div class=\"Title\">";
	$str_destacado .= "			<span class=\"count\">1.</span>";
	$str_destacado .= "			<a href=\"$link_paquete\">The Grand Del Mar</a>"; 
	$str_destacado .= "		</div>";*/
	
	$str_destacado .= "		<div class=\"wrapper\">";
	$str_destacado .= "			<div style=\"margin-left:65px;\" class=\"addtocart-area2\">";	
	
	$str_destacado .= "<form method=\"post\" class=\"product\" action=\"$link_paquete\">";
	$str_destacado .= "		<div class=\"addtocart-bar2\">";
	$str_destacado .= "			<div class=\"clear\"></div>";
	$str_destacado .= "			<span class=\"addtocart-button\">";
	$str_destacado .= "				<input type=\"submit\" name=\"bb\" class=\"addtocart-button cart-click\" value=\"Detalles\" title=\"Add to cart\">";
	$str_destacado .= "			</span>";
	$str_destacado .= "			<div class=\"clear\"></div>";
	$str_destacado .= "		</div>";
	$str_destacado .= "<form>";
...
if($languages->IsExistLanguage($languages_id)) {
	 ...
	 $SQL .= " tbl_paquete_details.language_id = '".$languages_id."' AND int_status='1' AND tbl_paquete.fk_categoria = '".$categoria."' ORDER BY tbl_paquete.pk_paquete DESC LIMIT ".$from.",".$to." ";
	...
	# exit();
	 ...
	 if($count >= 1) {
		...
		while($Fetch = $GLOBALS['CONNECT_DB']->Fetch($Query)) {
		    ...
		    $strgal_paquete .= "<li class=\"ajax_block_product bordercolor\">";
			$strgal_paquete .= "<a href=\"$link_paquete\" class=\"product_img_link\" title=\"$title_paquete\">";
			$strgal_paquete .= tep_image(_URL.'resize.php?image='.$img_thumb.'&w=180&h=180&IsCrop=0',$title_paquete,'','','');
			$strgal_paquete .= "</a>";
			$strgal_paquete .= "<div class=\"center_block\">";
			$strgal_paquete .= "<div class=\"product_flags\">";
			$strgal_paquete .= "<span class=\"new\">Oferta!</span> <span class=\"availability bordercolor\">Disponible";
			$strgal_paquete .= "</span> </div>";
			$strgal_paquete .= "<h3><a class=\"product_link\" href=\"$link_paquete\" title=\"Cancun JUNGLE TOUR\">".$title_paquete."</a></h3>";
			$strgal_paquete .= "<p class=\"product_desc\"><a class=\"product_descr\" href=\"$link_paquete\">".$Boleto." - ".$Traslate." / ".$noches." Noches - ".$dias." Dias -  ".$Presentacion."</a></p>";
			$strgal_paquete .= "</div>";
			$strgal_paquete .= "<div class=\"right_block bordercolor\">";
			$strgal_paquete .= "<span class=\"on_sale\">Precio Desde:</span>";
			$strgal_paquete .= "<span class=\"price\">$ ".$Fetch['txt_precio']."</span>";
			$strgal_paquete .= "<a class=\"ajax_add_to_cart_button exclusive\" href=\"$link_paquete\" title=\"Add to cart\">MAS DETALLES</a>";
			$strgal_paquete .= "</div>";
			$strgal_paquete .= "</li>";
		...
		$i=0;
	 		while($Fetch = $GLOBALS['CONNECT_DB']->Fetch($SQL)){
				...
				if(_SEOMOD==1){
					...
					$link_categoria = _URL.'tours/paquetes-'.$SafeName.'-pid-'.$Fetch['pk_categoria'].'.'._FEXT;
				}else{
