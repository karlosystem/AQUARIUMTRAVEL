...
function ListPaquetesCategoria($from = 0, $to = 20){
 ...
 if($count >= 1) {
	...
	while($Fetch = $GLOBALS['CONNECT_DB']->Fetch($Query)) {
		 ...
		 $strgal_paquete .= "		<span class=\"PricesalesPrice\" style=\"color:#d10c51;\">$ ";
         ...
         $strgal_paquete .= "						<input type=\"submit\" class=\"addtocart-button cart-click\" style=\"background-color: #302359;\" value=\"DETALLES\" title=\"$Title\" />";
		 $strgal_paquete .= "					</span>";
         $strgal_paquete .= "              </div>";                                              
         $strgal_paquete .= "			</form>";
         $strgal_paquete .= "           </div>";                                    
		 ...
		 $strgal_paquete .= "	<h2><a href=\"$link_paquete\">".$denominacion.$Title."</a></h2>";
		 ...
		 $strgal_paquete .= "			<a href=\"$link_paquete\">".$Dias." - ".$Noches."</a>";
		 $strgal_paquete .= "		</div>";                                            
		 $strgal_paquete .= "	</div>";
		 $strgal_paquete .= "</div>";
		 $strgal_paquete .= "<div class=\"clear\"></div>";
...
function DestacadosPaquetePortada($from = 0,$to = 0, $tipo){
 $SQL =  "SELECT 	tbl_paquete.txt_precio, tbl_paquete_details.fk_paquete, tbl_paquete.int_ishome, ";
 $SQL .= "	tbl_paquete_details.txt_detalle, tbl_paquete_details.txt_title, ";
 $SQL .= "	tbl_paquete_details.txt_incluye, tbl_categoria.fk_categoria, "; 
 $SQL .= "	txt_precio_soles, int_dias, int_noches "; 
 $SQL .= "FROM ((tbl_paquete ";
 $SQL .= " Inner Join tbl_paquete_details ON tbl_paquete.pk_paquete = tbl_paquete_details.fk_paquete)"; 
 $SQL .= " Inner Join tbl_categoria ON tbl_paquete.fk_categoria = tbl_categoria.pk_categoria) ";
 $SQL .= "WHERE ((tbl_paquete.int_isdestacado = '1' AND tbl_paquete_details.language_id = '2') AND int_status='1')";
 if($tipo == 1){
 $SQL .= "	AND tbl_categoria.fk_categoria in (3, 6, 5, 84)";
 }elseif($tipo == 2){
 $SQL .= "	AND tbl_categoria.fk_categoria not in (3, 6, 5, 84)";	 
 }
 $SQL .= "ORDER BY tbl_paquete.pk_paquete DESC LIMIT ".$from.",".$to." ";
 ...
 while($Fetch = $GLOBALS['CONNECT_DB']->Fetch($Query)){
	...
	if($ncounter==1 || $ncounter>3){$str_destacado .= "<li style=\"border: 0px;\" class=\"item\">";$ncounter = 1;}
	...
	$str_destacado .= "<div class=\"product-box spacer\" style=\"height: 320px;\">";
	$str_destacado .= "	<div class=\"browseImage\">";
	$str_destacado .= "		<div class=\"sale\"></div>";
	$str_destacado .= "		<a href=\"$link_paquete\" class=\"img2\">";
	$str_destacado .= tep_image(_URL.'resize.php?image='.$img_thumb.'&w=220&h=220&IsCrop=0',$title_paquete,'','','class="browseProductImage featuredProductImage"');
	
	$str_destacado .= "	    </a>";
	$str_destacado .= "		<div class=\"Price\">";
	
		$str_destacado .= "		<span class=\"sales\" style=\"color:#d10c51; font-size:14px; line-height: 15px; text-align:center;\">Desde $ ";
		$str_destacado .= 				round(str_replace(',', '', $Fetch['txt_precio']))." &oacute; S/.".round(str_replace(',', '', $Fetch['txt_precio_soles']));
		$str_destacado .= "<br>";
	...
	$str_destacado .= "			<div style=\"margin-left:50px;\" class=\"addtocart-area2\">";	
	$str_destacado .= "			<span class=\"addtocart-button\">";
	$str_destacado .= "				<a href=\"$link_paquete\" title=\"$title_paquete\" class=\"addtocart-button cart-click\">";
	$str_destacado .= "					<img src=\""._URL."/images/boton_ver_detalles.png\"></a>";
	$str_destacado .= "			</span>";
