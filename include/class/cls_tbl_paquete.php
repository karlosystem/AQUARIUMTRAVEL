...
var $pref_contenido = 'contenido_paquete';
...
{
	if(!tep_not_null($sql)){
	$sql = $GLOBALS['CONNECT_DB']->Query("SELECT pk_paquete, txt_datepaquete, fk_categoria, txt_youtube, int_status, txt_dateadd, txt_dateupdate, txt_date_from, txt_date_to, int_dias, int_noches, txt_precio, int_ishome, int_isnuevo, [|PREFIX|]paquete_details.txt_pdf, fk_destino, txt_precio_soles, txt_tipo_cambio, int_isdestacado, LENGTH(txt_meta_description) as caracteres, int_visto FROM [|PREFIX|]paquete LEFT JOIN [|PREFIX|]paquete_details ON [|PREFIX|]paquete.pk_paquete = [|PREFIX|]paquete_details.fk_paquete ORDER BY int_status ASC");
	}else{
	...
	while($Fetch = $GLOBALS['CONNECT_DB']->Fetch($sql))
	{
		...
		$arreglo[$i]['int_visto'] = $Fetch['int_visto'];
	 $i++;
	}
...
function Paquete_US($IsMode='Create'){#Banner_UpdateSave
		  $languages = language::tep_get_languages();
		   for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
			 ...
			 $contenido_paquete = secure_sql($_POST[$this->pref_contenido][$language_id]);			
			...
			$sql_data_array = array('fk_paquete' => $paquete_id,
				          ...
				          'txt_title' => $name_paquete,
									...
									'txt_contenido' => $contenido_paquete,
...
function get_infolang_pack($language_id = ''){
	...
	$sql = "SELECT txt_title, txt_incluye, txt_presentacion, txt_contenido, txt_detalle,txt_title_youtube,txt_descyoutube,txt_boleto,txt_traslate,txt_imagen, txt_pdf, txt_meta_title, txt_meta_description, txt_meta_keyword, txt_tours FROM";
	...
	$get_data_array[] = array('title' => $FetchInfo['txt_title'],
							  ...
							  'contenido' => $FetchInfo['txt_contenido'],
...
public static function get_paquete_detail($pak_id, $language_id = 2) {
    ...
    $paquete_query = "SELECT txt_title, txt_presentacion, txt_contenido, txt_detalle,txt_title_youtube,txt_descyoutube,txt_boleto,txt_traslate,txt_imagen, txt_pdf, txt_meta_title, txt_meta_description, txt_meta_keyword, txt_tours, txt_incluye FROM ";
    ...
    $getpaquete[] = array('title' => $FetchPaquete['txt_title'],
						 ...
						 'contenido' => $FetchPaquete['txt_contenido'],
...
function paquetes_relacionados_animado($categoria,$from=0,$to){
	 ...
	 $SQL = "SELECT tbl_paquete.txt_precio, tbl_paquete_details.fk_paquete, tbl_paquete.int_ishome, tbl_paquete_details.txt_detalle, tbl_paquete_details.txt_title, tbl_paquete_details.txt_incluye, tbl_paquete.int_dias, tbl_paquete.int_noches, txt_precio_soles FROM tbl_paquete ";
	 ...
	 if($count >= 1) {
		...
		$ncounter = 1;
			...
			while($Fetch = $GLOBALS['CONNECT_DB']->Fetch($Query)) {
				...
				if($ncounter==1 || $ncounter > 4){$strgal_paquete .= "<li class=\"item\">";$ncounter = 1;}
				...
				if($ImgPaq=='' || !file_exists($folder_complete.$ImgPaq))
					...
					$strgal_paquete .= "<div class=\"product-box spacer\" style=\"height: 300px;\">";
					$strgal_paquete .= "	<div class=\"fleft\">";
					$strgal_paquete .= "		<div class=\"description\" style=\"padding: 5px;\">";
					$strgal_paquete .= "			<p>".$title_paquete."</p>";
					$strgal_paquete .= "		</div>";
					$strgal_paquete .= "	</div>";
				
					$strgal_paquete .= "	<div class=\"browseImage\">";
					$strgal_paquete .= "		<div class=\"sale\"></div>";
					$strgal_paquete .= "		<a href=\"$link_paquete\">";
					$strgal_paquete .= tep_image(_URL.'resize.php?image='.$img_thumb.'&w=208&h=220&IsCrop=0',$title_paquete,'','','class="browseProductImage featuredProductImage"');	
					$strgal_paquete .= "	    </a>";
					$strgal_paquete .= "	</div>";
				
					$strgal_paquete .= "	<div style=\"background-color: #eeeeee; padding-top:5px; margin-top:-15px !important;\">";
					$strgal_paquete .= "		<div class=\"Price\">";
					$strgal_paquete .= "		<span class=\"sales\" style=\"margin-top:1px !important; padding-top: 5px;\">";
					$strgal_paquete .= "    Desde $ <span style=\"font-size:18px; font-weight:bold\">";
					$strgal_paquete .= 				round(str_replace(',', '', $Fetch['txt_precio']))."</span>";
					$strgal_paquete .= 			" &oacute; S/. <span style=\"font-size:18px; font-weight:bold\">".round(str_replace(',', '', $Fetch['txt_precio_soles'])). "</span>";
					$strgal_paquete .= "		</span>";
					$strgal_paquete .= "		<span class=\"frecuencia\">";
					$strgal_paquete .= 				$Fetch['int_dias']." Dias / ".$Fetch['int_noches']." Noches" ;
					$strgal_paquete .= "		</span>";		
					$strgal_paquete .= " </div>";
					
					$strgal_paquete .= "		<div class=\"wrapper\">";
					$strgal_paquete .= "			<div style=\"margin-left:25px;\" class=\"addtocart-area2\">";	
					$strgal_paquete .= "				<span class=\"addtocart-button\">";
					$strgal_paquete .= "					 <button class=\"btn-hover color-11\">";
					$strgal_paquete .= "						<a style=\"color:inherit\" href=\"$link_paquete\">VER OFERTA</a>";
					$strgal_paquete .= " 				</button>";
					...
					if($ncounter==$ncol) $strgal_paquete .= "</li>";
  				$ncounter++; 	 
...
public function actualizarVisitante(){
  ...
  {
    $insertview = mysql_query("insert into tbl_pageview values('','yourpage','$user_ip')");
		$updateview = mysql_query("update tbl_totalview set totalvisit = totalvisit+1 where page='yourpage' ");
  }
}
