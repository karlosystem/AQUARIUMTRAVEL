...
class file_upload {
	...
	var $the_file_size;
	var $max_size = 1024000; #1Mb <> 1024*1000
	...
	function check_file_name($the_name) { #Inicio de la funcion
		$file_size = $this->the_file_size; # obtengo el tamaño del archivo.
		if ($the_name != "") {
	...
	function error_text($err_num) {
		switch ($this->language) {
			...
			case "es":
			$error[0] = "Archivo: <b>".$this->the_file."</b> ha cargado correctamente!"; 
			$error[1] = "El archivo subido supera el máximo tamaño permitido. Cargar archivo de directiva en la configuración del servidor."; 
			$error[2] = "El archivo subido supera la directiva que se especificó en el formulario ."; 
			$error[3] = "El archivo subido fue subido sólo parcialmente"; 
			$error[4] = "El archivo no se ha subido"; 
			$error[10] = "Por favor, seleccione un archivo para cargar."; 
			$error[11] = "Sólo los archivos con las siguientes extensiones se permite: <b>".$this->ext_string."</b>";
			$error[12] = "Lo siento, el nombre del archivo contiene caracteres no válidos. Utilice únicamente caracteres alfanuméricos y separar las partes del nombre (si es necesario) con un guión bajo. Un nombre de archivo válido termina con un punto seguido de la extensión."; 
			$error[13] = "El nombre del archivo supera la longitud máxima de ".$this->max_length_filename." caractéres."; 
			$error[14] = "Lo sentimos, pero la carga directorio no existe!"; 
			$error[15] = "Subiendo <b>".$this->the_file."...error!</b> Lo sentimos, el nombre del archivo ya existe";
			$error[16] = "El archivo subido es renombrado a <b>".$this->file_copy."</b>.";
			break;
